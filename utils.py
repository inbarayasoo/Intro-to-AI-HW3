...
ID = 214163735  # TODO: change it to your personal ID
...
def accuracy(y: np.array, y_pred: np.array)-> float:
    ...
    accuracy_val = None
    # ====== YOUR CODE: ======
    y_diff = (y == y_pred).astype(int)
    y_true_pred = y_diff.sum()
    accuracy_val = y_true_pred / y_diff.size
    # ========================
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    # ====== YOUR CODE: ======
    x1_matrix = np.tile(x1[:, np.newaxis, :], (1, x2.shape[0], 1))
    x2_matrix = np.tile(x2[np.newaxis, :, :], (x1.shape[0], 1, 1))
    dists = np.linalg.norm(x1_matrix - x2_matrix, axis=2)
    # ========================
